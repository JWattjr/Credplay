...
import CredPlayAgentPanel from "@/components/markets/CredPlayAgentPanel";
...
import { calculateGrossUsdt, calculateTradingFee, formatTradingFee } from "@/lib/fees";
import {
  addComment,
...
export default function MarketDetail({ marketId }: MarketDetailProps) {
  ...
  const tradeTotal = market && validTradeAmount
    ? tradeAction === "BUY"
      ? calculateGrossUsdt(tradeAmountNumber, market.trading_fee_bps)
  ...
  const executeTrade = useCallback(async (side: VoteSide) => {
    ...
    await runAction(async () => {
      void side;
      throw new Error("USDT trading is pending review and not active in this phase.");
    });
  }, [market, runAction]);
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <CredPlayAgentPanel market={market} />
  ...
  return (
    <section className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        ...
        <span className="font-mono text-[11px] text-[var(--muted)]">USDT</span>
      </div>
      ...
      <label className="mb-2 block font-mono text-[11px] font-bold uppercase text-[var(--muted)]" htmlFor="market-trade-amount">
        {action === "BUY" ? "Amount (USDT)" : `Shares to sell (${selectedSide})`}
      </label>
      <input
      ...
      <div className="mt-3 grid gap-1 font-mono text-[11px] text-[var(--muted)]">
        ...
        <div className="flex justify-between">
          ...
          <span>{action === "BUY" ? estimatedShares.toFixed(4) : `${sellProceeds.toFixed(4)} USDT`}</span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between text-[var(--foreground)]">
          ...
          <span>{action === "BUY" ? total.toFixed(4) : netProceeds.toFixed(4)} USDT</span>
        </div>
      </div>
      ...
      {action === "SELL" && (
        <p className="mt-3 rounded-[7px] border border-dashed border-[var(--border)] bg-[var(--surface-muted)] p-3 text-xs leading-relaxed text-[var(--muted)]">
          MVP sell orders update your in-app ledger position. On-chain USDT withdrawals require the future market escrow contract.
        </p>
      )}
    ...
    <section className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-5 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 font-mono text-[11px] text-[var(--muted)]">USDT-backed opinions only</p>
        </div>
      ...
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-[var(--muted)]">
            Prediction posts pay an X Layer creation fee before entering social qualification.
          </p>
        </div>
      ...
      <div className="grid gap-2 font-mono text-xs text-[var(--muted)]">
        ...
        <div className="flex flex-wrap justify-between gap-3 border-t border-dashed border-[var(--border)] pt-3">
          ...
          <span className="text-[var(--foreground)]">
            {isTradable ? "Approved for USDT trading" : canApprove ? "Ready for approval" : "Waiting for qualification"}
          </span>
        </div>
      </div>
...
function MarketStatsPanel({
  ...
  return (
    <section className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm">
      ...
      <StatRow label="Liquidity" value={`${liquidity.toLocaleString(undefined, { maximumFractionDigits: 2 })} USDT`} />
      <StatRow label="Volume" value={`${volume.toLocaleString(undefined, { maximumFractionDigits: 2 })} USDT`} />
...
function CreatorPanel({
  ...
  return (
    <section className="rounded-[12px] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-sm">
      ...
      <StatRow label="Visible volume" value={`${totalVolume.toLocaleString(undefined, { maximumFractionDigits: 2 })} USDT`} />
