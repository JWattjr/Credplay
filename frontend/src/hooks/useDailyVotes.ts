...
export function useDailyVotes(userId?: string) {
  ...
  const reload = useCallback(async () => {
    ...
    try {
      ...
      setError(caught instanceof Error ? caught.message : "Unable to load daily free calls.");
    } finally {
      setLoading(false);
    }
  }, [userId]);
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (active) setError(caught instanceof Error ? caught.message : "Unable to load daily free calls.");
      } finally {
